...
import { FaPaperPlane, FaRobot, FaBrain, FaExclamationTriangle, FaSmile, FaImage } from 'react-icons/fa';
...
export default function VirtualAgent({ onClose }) {
  ...
  useEffect(() => {
    if (activeModel === 'gemini') {
      setMessages([{ 
        from: 'agent', 
        text: "👋 Hi! I'm your AI Chat assistant. How can I help you today?" 
      }]);
      ...
      setMessages([{ 
        from: 'agent', 
        text: "👋 Hi! I'm your ServiceNow Virtual Assistant. What would you like help with today?",
        options: [
          { text: '📦 Product Offerings', value: 'product_offerings' },
          { text: '📑 Product Specifications', value: 'product_specifications' },
          { text: '⚠️ Incidents', value: 'incidents' },
    ...
    try {
      ...
      switch (conversationState.stage) {
        case 'category_selection':
          ...
          if (['product_offerings', 'product_specifications', 'incidents'].includes(selectedCategory)) {
            setConversationState({
              ...conversationState,
            ...
            setMessages(prev => [...prev, {
              from: 'agent',
              text: "I can help you with Product Offerings, Product Specifications, or Incidents. Which one would you like?",
              options: [
                { text: '📦 Product Offerings', value: 'product_offerings' },
                { text: '📑 Product Specifications', value: 'product_specifications' },
                { text: '⚠️ Incidents', value: 'incidents' }
  ...
  return (
    <div className="fixed bottom-34 right-6 w-96 h-[600px] rounded-3xl overflow-hidden shadow-lg z-50 flex flex-col font-sans border border-gray-200 bg-white">
      {/* Header */}
      <div className="bg-gradient-to-r from-[#00B4D8] to-[#0077B6] text-white px-5 py-4 flex justify-between items-center">
        <div className="flex items-center gap-2">
          ...
          <button 
            ...
            title={`Switch to ${activeModel === 'gemini' ?'AI Chat' : 'ServiceNow'}`}
            disabled={isLoading}
          >
            {activeModel === 'gemini' ? <FaRobot /> : <FaBrain />}
            {activeModel === 'gemini' ? 'AI Chat' : 'ServiceNow'}
          </button>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3 bg-gradient-to-b from-white via-[#f8fbfc] to-white">
        {messages.map((msg, idx) => (
          <div key={idx}>
            <div
              className={`max-w-[80%] px-4 py-2 text-sm rounded-2xl shadow mb-2 ${
                msg.from === 'agent'
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {msg.options.map((option, i) => (
                  <button
                    ...
                    className="px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-full text-sm border border-gray-300"
                    disabled={isLoading}
                  >
                    {option.text}
                  </button>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              ...
              className="text-[#00B4D8] hover:text-[#00B4D8] p-2 rounded-full hover:bg-[#00B4D8]-100 transition"
            >
              <FaSmile />
            </button>
            
            {showEmojiPicker && (
              <div className="absolute bottom-12 left-0 bg-white border border-gray-200 rounded-lg shadow-lg p-2 z-10 w-48 grid grid-cols-5 gap-1">
          ...
          <button
            ...
            className={`bg-gradient-to-r from-[#00B4D8] to-[#0077B6] text-white p-2 rounded-full transition transform shadow-md ${
              isLoading || (!input.trim() && !attachment) ? 'opacity-50' : 'hover:scale-105'
            }`}
          >
            <FaPaperPlane />
          </button>
